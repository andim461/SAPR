...
const calc = (store: Store): Solve => {
    ...
    const deltas = gaussJordan(matrixA, vectorB).map((val) => Number(val.toFixed(4)));

    console.log(`deltas: ${deltas}`);

    const U: Array<(x: number) => number> = [];
    rods.forEach((val, ind) => {
        const Ux = (x: number) => ((1- x / val[2]) * deltas[ind] + x/val[2] * deltas[ind + 1] + (rodsLoads[ind] * Math.pow(val[2], 2) * x / (2 * val[0] * val[1] * val[2])) *(1 - x * val[2]));
        U.push(Ux);
    }
    );
    console.log(U);
    ...
    rods.forEach((val, ind) =>{
        const Nx = (x: number) => (val[0]*val[1] / val[2]) * (deltas[ind + 1] - deltas[ind]) + ((rodsLoads[ind] * val[2]) / 2) * (1 - 2 * (x / val[2]));
        N.push(Nx);
    });
    console.log(N);
